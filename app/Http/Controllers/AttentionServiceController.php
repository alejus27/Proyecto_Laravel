...
use App\Models\Attention_service;
use Illuminate\Http\Request;

class AttentionServiceController extends Controller
{
    public function index(){
        return response()->json(Attention_service::all(),200);   //Hace referencia al modelo
    }
    
    //Mostrar un solo elemento filtrado por el id
    public function show($id){
        $the_cartMedicine=Attention_service::find($id);
    ...
    public function store(Request $request){
        $the_cartMedicine=Attention_service::create($request->all());
